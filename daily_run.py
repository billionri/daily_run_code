...
# var2 = "\\Non_Algo_56548_INST_Trades.xlsx" "Z:\APITesting\TG - Enc\ETI_multi_response-encryption\ETI_UAT.xlsx"
var2 = "ETI_UAT_NonAlgo_42718.xlsx"
...
#     port_SL = 19306
#     MasterData_Path = r"D:\Monika\Renish\trade_matching - UAT.xlsx"
# if Env.upper() == "NFR" or Env.upper() == "NFRA":
#     print("NFR Env selected")
#     ip_CG = '192.168.80.50'
#     port_CG = 19006
#     ip_SL = '192.168.80.50'
#     port_SL = 19006
#     MasterData_Path = r"C:\Users\ext-rashida\Desktop\ETI_NFR_NonAlgo_56191_AN_Rashid.xlsx"
...
#     ip_SL = '192.168.80.50'
#     port_SL = 19006
#     MasterData_Path = r"D:\Moin KT\Daily\ETI_NFR_NonAlgo_56191_AN.xlsx"
#     password_key: bytes = b'Admin@22200000000000000000000000'
#     password_key = b'Admin@22200000000000000000000000'
#     password_iv = b'Admin@222000'
#
...
# print(port_SL)
print(conn_variables[Env]["port_CG_and_port_SL"])

# MasterData_Path = r"\\172.22.0.14\ext-home$\ext-ganeshko\Desktop\MAT Data\imp working with UAT-10002.xlsx"
# print(MasterData_Path)
# print(conn_variables[Env]["MasterData_Path"])
...
df_CG = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_CG, engine='openpyxl')
rqstFields_CG = df_CG.loc[sheetRow_CG].iloc[2:14]

# Reading Session Logon Data
df_SL = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_SL, engine='openpyxl')
rqstFields_SL = df_SL.loc[sheetRow_SL].iloc[8:24]
rqstFields_SL_header = df_SL.loc[sheetRow_SL].iloc[2:8]
# Reading User Logon Data
df_UL = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_UL, engine='openpyxl')
...
df_Sub = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_Sub, engine='openpyxl')
...
df_ULT = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_ULT, engine='openpyxl')
rqstFields_ULT = df_ULT.loc[sheetRow_ULT].iloc[2:10]

# Reading Session Logout Data
df_SLT = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_SLT, engine='openpyxl')
rqstFields_SLT = df_SLT.loc[sheetRow_SLT].iloc[2:8]

# Reading Other API to be tested
df_OtherAPI = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_Orderdata, engine='openpyxl')
...
excel_Report_Path = ".\\Report\\{}{}".format(dt_string, var2)
...
def excel_report():
    ...
    for i in range(0, len(Order_num)):
        ...
        test_case_name4.value = str(TestCase_name_final[1])
        userid.value = str(user_id[1])
...
async def Read_Response(sock, key, iv):
    actul = " "
    Comment = " "
    heartbeat_counter = 0
    global noOfResponses
...
def generateEncryptedText(i, order_type, order_row_number, non_persistent_orderno):
    df_orderdata = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_Orderdata, engine='openpyxl')
    ...
    order_header_stream = CreatePayload(rqstFields_header, order_header)
    bytestream_order = CreatePayload(rqstFields_message, neworder_formattype_message)
    ########################################################################

    encrypted_text = encryptor.update(bytestream_order)
    encrypted_text1 = order_header_stream + encrypted_text

    sock.sendall(encrypted_text1)

    return rqstFields_header, rqstFields_message
...
def decode_response(resp_OtherAPI_Temp, resp_OtherAPI2):
    ...
    if (resp_OtherAPI_Temp == 10010):
        ...
        Comment = str(" Trade Notification => trade no. " + str(trade_data1[30]) + " Trade executed qty => " + str(
            trade_data1[26] / 10000) + " Trade pending qty =>  " + str(trade_data1[27]))
        ...
        decryption_message = decryptor.update(encrypted_msg)
        # print("decryption_message ",decryption_message)
        resp_OtherAPI = struct.unpack_from(
            'IQIH16sBBB7sQQQQQQQQQQQQQQQQQQQQQQQIIIIIIIHHBBBccBBBBBBBBB2sB5s7s9s2sB20s12s12s12sBBB2s',
            decryption_message)
...
async def sendOrder(sock, key, iv):
    # print("total no of orders available - ", order_count)
    print(conn_variables[Env]["MasterData_Path"])
    config_data = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name="Configuration", engine='openpyxl')
    print("total no of orders available - ", len(config_data))

    for i in range(0, len(config_data)):
        print(f"-----------------------------------Sending Order {i+1}------------------------------------")
        ...
        if status.upper() == "Y":
            ...
            rqstFields_header, rqstFields_message = await generateEncryptedText(i, order_type, order_row_number, non_persistent_orderno)
            print(rqstFields_header, rqstFields_message)
            # if order_type.upper() == "NEW":
            #     df_orderdata = pd.read_excel(open(conn_variables[Env]["MasterData_Path"], 'rb'), sheet_name=sheetName_Orderdata,
            #                                  engine='openpyxl')
            #     config_data_read = df_orderdata.loc[order_row_number]
            #     # print("new order data template", config_data_read[1])
            #     user_id.append(config_data_read[5])
            #     message_seq_no.append(config_data_read[4])
            #     print("order Type is ", order_type, " & order no is ", i + 1, " & Message seq num is ",
            #           config_data_read[4])
            #     rqstFields_header = config_data_read.loc["BodyLen":"Price"].iloc[0:6]
            #     rqstFields_message = config_data_read.loc["Price":"FreeText3"]
        ...
        await Read_Response(sock, key, iv)


# Gateway Session Connection
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
    sock.connect((conn_variables[Env]["ip_CG_and_ip_SL"], conn_variables[Env]["port_CG_and_port_SL"]))
    print("-----------------------------------------------------------------------")
    ...
    if (resp_SL_Temp[1] == 10010):
        ...
        key = conn_variables[Env]["password_key"]
        iv = conn_variables[Env]["password_iv"]
        encrypted_msg = byte_array
        # byte_array = bytearray(byte_string)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        if (resp_SL_Temp[1] == 10010):
            ...
            encrypted_msg = byte_array
            decryption_message = decryptor.update(encrypted_msg)
            resp_SL = struct.unpack_from('QQQQQQIB3sIHB1s{}s'.format(len(decryption_message) - 64),
        ...
        try:
            ...
            asyncio.run(sendOrder(sock, final_key, final_iv))
